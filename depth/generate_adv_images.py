...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        depth_pil = Image.open(self.image_paths[i])
        ...
        try:
            ...
            print(f'Error norm, path is {self.image_paths[i]}, setting depth to zero. \nThis file will be omitted in the main loop.')
            depth_image_inv = np.zeros(inv_gt_norm.shape).astype(np.uint8)
