...
class TextualInversionDataset(Dataset):
    def __init__(
        ...
        self.cond_data_root = cond_data_root
        ...
        self.taskonomy_split_path = taskonomy_split_path
        ...
        with open(self.data_paths_file[0], 'r') as f:
            data = f.readlines()
        data = [x.strip().replace('depth_zbuffer', 'rgb') for x in data]

        split = pd.read_csv(self.taskonomy_split_path)
        train_buildings = split[split['train'] == 1]['id'].values.tolist()
        val_buildings = split[split['val'] == 1]['id'].values.tolist()
        train_buildings.remove('darnestown') # for that building there is no mask..
        val_buildings.remove('tomkins') # for that building there is no mask..

        train_image_paths = [x for x in data if x.split('/')[-3].split('_')[0] in train_buildings]
        val_image_paths = [x for x in data if x.split('/')[-3].split('_')[0] in val_buildings]
    ...
    def __getitem__(self, i):
        ...
        image_depth_file = self.image_paths[i].split('/')[-1].replace('rgb','depth_zbuffer')

        building = self.image_paths[i].split('/')[-3].split('_')[0]
        mask_root_ = '/'.join(self.mask_data_root.split('/')[:-1])
        cond_root_ = '/'.join(self.cond_data_root.split('/')[:-2])
        image_depth = Image.open(os.path.join(cond_root_, f'{building}_depth_zbuffer/depth_zbuffer/', image_depth_file))
        image_mask = Image.open(os.path.join(mask_root_, f'{building}_mask_valid/', image_depth_file))
        ...
        example["gt"] = torch.from_numpy(np.clip(image_depth/8000,0.,1.))
        ...
        # inv_gt_norm = (255-(image_depth)).astype(float)
